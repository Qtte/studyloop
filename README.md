...
> ?????????? `MemoryTool`?`RAGTool`?`NoteTool`?`TerminalTool` ???????????????????? `target.md` ??????????? MVP ??????????????????????

### ??????

- ???????[backend/app/services/study_context_builder.py](backend/app/services/study_context_builder.py)
- ??????`LearningContextPacket`?`StudyContextConfig`?`StudyContextBuilder`
- ?????`gather -> select -> structure -> compress`

### ? HelloAgents ???????

????????? `hello_agents/context/builder.py` ? GSSC ????????????????????? 10 ????
...
????????????????? `MemoryTool/RAGTool` ???????? `StudyAgentService` ?????

1. `SimpleKeywordRetriever.search()` ?????
2. `LearningNoteTool.search_notes()` ???????
3. `LearningNoteTool.list_notes(note_type="mistake_record")` ???????
4. `mastery_by_topic` ????????
5. API ??????????????????
...
1. ? `SimpleKeywordRetriever` ??????????? `StudyContextBuilder` ??????
2. ? `LearningNoteTool` ??????????????????????
3. ? `compress()` ????????????????? 10 ? section header?
...
## StudyLoop Backend MVP

A minimal FastAPI backend is available under `backend/app` and reuses the HelloAgents architecture for:
- structured study context building
- markdown learning notes
- lightweight keyword retrieval
- mastery score updates
- mock LLM fallback when no OpenAI key is configured
...
Add these values to `.env` when you want to use a real model:
...
When `OPENAI_API_KEY` is missing or `USE_MOCK_LLM=true`, the backend uses a deterministic mock model for local testing.

### Example Flow

1. `POST /materials/ingest` to add study material.
2. `POST /study/explain` to request an explanation using retrieved evidence.
3. `POST /study/quiz` to generate a quiz in JSON.
4. `POST /study/grade` to grade an answer and create a `mistake_record` note.
5. `GET /notes` and `GET /study/state` to inspect saved notes and current mastery state.

## StudyLoop LangGraph Phase

The second StudyLoop phase upgrades the backend from direct service calls to a LangGraph-orchestrated learning loop.
...
- `parse_user_intent`: normalizes the task into `explain`, `quiz`, or `grade`.
- `retrieve_materials`: runs lightweight retrieval over ingested learning chunks.
- `retrieve_learning_notes`: loads notes and mistake records for the current topic.
- `build_study_context`: composes the 10-section study context using `StudyContextBuilder`.
- `explain_concept`: produces a grounded explanation.
- `generate_quiz`: returns a structured `QuizQuestion` payload.
- `grade_answer`: returns a structured `GradingResult` payload.
- `update_memory`: updates mastery and writes a `mistake_record` note.
- `replan_learning_path`: produces a structured `LearningPlan` for the next study step, and routes back to `generate_quiz` (remediation) when mastery stays below threshold.
- `remediation loop`: when `mastery_after < 0.6` after grading, `replan_learning_path` triggers one remediation quiz on the same topic (`retry_count` guards against infinite loops).

### API Integration

- `/study/explain` now uses the graph explain path.
- `/study/quiz` now uses the graph quiz path.
- `/study/grade` now uses the graph grade path and persists notes through the graph.
